namespace Biblioteca_de_Controles
{
    partial class ucDatosVuelo
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Component Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify 
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            PbModeloAvion = new PictureBox();
            LblModeloAvion = new Label();
            label1 = new Label();
            LblNombreCapitan = new Label();
            LblAeropuertoDespegue = new Label();
            label3 = new Label();
            LblAeropuertoAterrizaje = new Label();
            label4 = new Label();
            LblNumerodePasajeros = new Label();
            label5 = new Label();
            label2 = new Label();
            pbpiloto = new PictureBox();
            ((System.ComponentModel.ISupportInitialize)PbModeloAvion).BeginInit();
            ((System.ComponentModel.ISupportInitialize)pbpiloto).BeginInit();
            SuspendLayout();
            // 
            // PbModeloAvion
            // 
            PbModeloAvion.BorderStyle = BorderStyle.FixedSingle;
            PbModeloAvion.Image = Properties.Resources.avion;
            PbModeloAvion.Location = new Point(3, 36);
            PbModeloAvion.Margin = new Padding(3, 2, 3, 2);
            PbModeloAvion.Name = "PbModeloAvion";
            PbModeloAvion.Size = new Size(117, 75);
            PbModeloAvion.SizeMode = PictureBoxSizeMode.StretchImage;
            PbModeloAvion.TabIndex = 0;
            PbModeloAvion.TabStop = false;
            // 
            // LblModeloAvion
            // 
            LblModeloAvion.AutoSize = true;
            LblModeloAvion.BackColor = Color.Silver;
            LblModeloAvion.Font = new Font("Segoe UI", 10.2F, FontStyle.Regular, GraphicsUnit.Point, 0);
            LblModeloAvion.Location = new Point(12, 126);
            LblModeloAvion.Name = "LblModeloAvion";
            LblModeloAvion.Size = new Size(45, 19);
            LblModeloAvion.TabIndex = 1;
            LblModeloAvion.Text = "label1";
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.BackColor = SystemColors.ActiveCaption;
            label1.Font = new Font("Segoe UI Emoji", 9F, FontStyle.Bold);
            label1.Location = new Point(126, 41);
            label1.Name = "label1";
            label1.Size = new Size(64, 16);
            label1.TabIndex = 2;
            label1.Text = "CAPITAN";
            label1.TextAlign = ContentAlignment.BottomCenter;
            // 
            // LblNombreCapitan
            // 
            LblNombreCapitan.AutoSize = true;
            LblNombreCapitan.BackColor = Color.Silver;
            LblNombreCapitan.Font = new Font("Segoe UI", 10.2F, FontStyle.Regular, GraphicsUnit.Point, 0);
            LblNombreCapitan.Location = new Point(126, 66);
            LblNombreCapitan.Name = "LblNombreCapitan";
            LblNombreCapitan.Size = new Size(45, 19);
            LblNombreCapitan.TabIndex = 3;
            LblNombreCapitan.Text = "label1";
            // 
            // LblAeropuertoDespegue
            // 
            LblAeropuertoDespegue.AutoSize = true;
            LblAeropuertoDespegue.BackColor = Color.Silver;
            LblAeropuertoDespegue.Font = new Font("Segoe UI", 10.2F, FontStyle.Regular, GraphicsUnit.Point, 0);
            LblAeropuertoDespegue.Location = new Point(126, 120);
            LblAeropuertoDespegue.Name = "LblAeropuertoDespegue";
            LblAeropuertoDespegue.Size = new Size(45, 19);
            LblAeropuertoDespegue.TabIndex = 5;
            LblAeropuertoDespegue.Text = "label1";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.BackColor = SystemColors.ActiveCaption;
            label3.Font = new Font("Segoe UI Emoji", 9F, FontStyle.Bold);
            label3.Location = new Point(126, 93);
            label3.Name = "label3";
            label3.Size = new Size(160, 16);
            label3.TabIndex = 4;
            label3.Text = "Aeropuerto de Despegue";
            label3.TextAlign = ContentAlignment.BottomCenter;
            // 
            // LblAeropuertoAterrizaje
            // 
            LblAeropuertoAterrizaje.AutoSize = true;
            LblAeropuertoAterrizaje.BackColor = Color.Silver;
            LblAeropuertoAterrizaje.Font = new Font("Segoe UI", 10.2F, FontStyle.Regular, GraphicsUnit.Point, 0);
            LblAeropuertoAterrizaje.Location = new Point(126, 178);
            LblAeropuertoAterrizaje.Name = "LblAeropuertoAterrizaje";
            LblAeropuertoAterrizaje.Size = new Size(45, 19);
            LblAeropuertoAterrizaje.TabIndex = 7;
            LblAeropuertoAterrizaje.Text = "label1";
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.BackColor = SystemColors.ActiveCaption;
            label4.Font = new Font("Segoe UI Emoji", 9F, FontStyle.Bold);
            label4.Location = new Point(126, 146);
            label4.Name = "label4";
            label4.Size = new Size(159, 16);
            label4.TabIndex = 6;
            label4.Text = "Aeropuerto de Aterrizaje";
            label4.TextAlign = ContentAlignment.BottomCenter;
            // 
            // LblNumerodePasajeros
            // 
            LblNumerodePasajeros.AutoSize = true;
            LblNumerodePasajeros.BackColor = Color.Silver;
            LblNumerodePasajeros.Font = new Font("Segoe UI", 10.2F, FontStyle.Regular, GraphicsUnit.Point, 0);
            LblNumerodePasajeros.Location = new Point(12, 180);
            LblNumerodePasajeros.Name = "LblNumerodePasajeros";
            LblNumerodePasajeros.Size = new Size(45, 19);
            LblNumerodePasajeros.TabIndex = 9;
            LblNumerodePasajeros.Text = "label1";
            // 
            // label5
            // 
            label5.BackColor = SystemColors.ActiveCaption;
            label5.Font = new Font("Segoe UI", 10.2F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label5.Location = new Point(12, 152);
            label5.Name = "label5";
            label5.Size = new Size(77, 17);
            label5.TabIndex = 8;
            label5.Text = "Pasajeros";
            label5.TextAlign = ContentAlignment.BottomLeft;
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.BackColor = Color.Yellow;
            label2.Font = new Font("Segoe UI Emoji", 10.2F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label2.ForeColor = SystemColors.ActiveCaptionText;
            label2.Location = new Point(0, 0);
            label2.Name = "label2";
            label2.Size = new Size(25, 19);
            label2.TabIndex = 10;
            label2.Text = "ID";
            label2.TextAlign = ContentAlignment.BottomLeft;
            // 
            // pbpiloto
            // 
            pbpiloto.BorderStyle = BorderStyle.FixedSingle;
            pbpiloto.Image = Properties.Resources.avion;
            pbpiloto.Location = new Point(199, 15);
            pbpiloto.Margin = new Padding(3, 2, 3, 2);
            pbpiloto.Name = "pbpiloto";
            pbpiloto.Size = new Size(86, 70);
            pbpiloto.SizeMode = PictureBoxSizeMode.StretchImage;
            pbpiloto.TabIndex = 11;
            pbpiloto.TabStop = false;
            // 
            // ucDatosVuelo
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            BackColor = Color.White;
            Controls.Add(pbpiloto);
            Controls.Add(label2);
            Controls.Add(LblNumerodePasajeros);
            Controls.Add(label5);
            Controls.Add(LblAeropuertoAterrizaje);
            Controls.Add(label4);
            Controls.Add(LblAeropuertoDespegue);
            Controls.Add(label3);
            Controls.Add(LblNombreCapitan);
            Controls.Add(label1);
            Controls.Add(LblModeloAvion);
            Controls.Add(PbModeloAvion);
            Margin = new Padding(3, 2, 3, 2);
            Name = "ucDatosVuelo";
            Size = new Size(293, 201);
            ((System.ComponentModel.ISupportInitialize)PbModeloAvion).EndInit();
            ((System.ComponentModel.ISupportInitialize)pbpiloto).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private PictureBox PbModeloAvion;
        private Label LblModeloAvion;
        private Label label1;
        private Label LblNombreCapitan;
        private Label LblAeropuertoDespegue;
        private Label label3;
        private Label LblAeropuertoAterrizaje;
        private Label label4;
        private Label LblNumerodePasajeros;
        private Label label5;
        private Label label2;
        private PictureBox pbpiloto;
    }
}
